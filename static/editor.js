...
(function () {
    ...
    canvas.addEventListener('mousedown', (e) => {
        ...
        if (currentTool === 'delete') {
            if (n) {
                if (defaultIds.has(n.id)) { alert('Нельзя удалить системный нейрон'); return; }
    ...
    canvas.addEventListener('mouseup', (e) => {
        ...
        if (drawingFrame) {
            ...
            window.appSocket.emit('add_group', {
                id, x: x0, y: y0, w, h, label: 'Группа', color: '#3a5cff', comment: '',
            });
            drawingFrame = null;
            return;
        }
    ...
    function draw() {
        ...
        ctx.fillText(
            `zoom ${view.scale.toFixed(2)}× · колесо=зум · Shift+drag=pan · Del=удалить выделение · нейронов ${topology.neurons.length} · импульсов ${pulses.from_idx.length}`,
            6, 6,
        );
    }
