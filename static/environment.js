...
(function () {
    ...
    function renderCartPole(s) {
        ...
        ctx.beginPath();
        ctx.moveTo(cxWorld, groundY - cartH);
        ctx.lineTo(px, py);
        ctx.stroke();
        ctx.lineCap = 'butt';
        ctx.fillStyle = '#ffd87a';
    ...
    function drawHud() {
        ...
        ctx.fillText(`zoom ${view.scale.toFixed(2)}× · колесо=зум · Shift+drag=pan`, 10, 14);

        if (state.kind === 'bug') {
            const a = state.agent;
            const x0 = 10, y0 = 36, w = 160, h = 12, gap = 6;
            drawBar(x0, y0, w, h, a.health, '#6ad06a', '#1f3a1f', 'HP');
            drawBar(x0, y0 + (h + gap), w, h, a.hunger, '#ffb14a', '#3a2a16', 'голод');
            drawBar(x0, y0 + 2 * (h + gap), w, h, a.fatigue, '#b681e7', '#2a1d3a', 'усталость');
            return;
        }
        ...
        if (task === 'cartpole') {
            drawHudLines([
                `шагов: ${state.steps ?? state.episode_steps ?? 0}`,
                `эпизод: ${state.episode ?? state.episode_index ?? 0}`,
                `прошлый: ${state.last_episode_steps ?? 0}`,
                `рекорд: ${state.best_steps ?? 0}`,
                `reward sensor: ${Number(state.reward_signal ?? 0).toFixed(2)}`,
                state.done ? doneReasonText(state.done_reason) : '',
            ], '#9bcaff');
        } else if (task === 'mountain_car') {
            const best = state.best_steps && state.best_steps > 0 ? state.best_steps : 'не решено';
            drawHudLines([
                `шагов: ${state.steps ?? state.episode_steps ?? 0}`,
                `эпизод: ${state.episode ?? state.episode_index ?? 0}`,
                `прошлый: ${state.last_episode_steps ?? 0}`,
    ...
    function doneReasonText(reason) {
        if (reason === 'goal') return '— цель! —';
        if (reason === 'time limit') return '— лимит шагов —';
        if (reason === 'fallen') return '— упал —';
        if (reason === 'terminated') return '— завершено —';
        return reason ? `— ${reason} —` : '— завершено —';
    }
    ...
    window.envView = {
        update(envSnap) {
            state = envSnap;
            const nextTask = state ? (state.kind === 'bug' ? 'bug' : gymTaskKey(state)) : null;
            if (nextTask && currentTaskKey && nextTask !== currentTaskKey) {
                view.scale = 1;
                view.tx = 0;
                view.ty = 0;
            }
            currentTaskKey = nextTask;
        },
    };
